...
const Home = () => {
  ...
  const initiateSearch = async (searchInput, countryCode) => {
    ...
    if (!isNaN(parseInt(searchInput))) {
      apiUrl = `https://api.openweathermap.org/data/2.5/weather?zip=${searchInput},${countryCode}&appid=${API_KEY}&units=metric`;
    } else {
      apiUrl = `https://api.openweathermap.org/data/2.5/weather?q=${searchInput},${countryCode}&appid=${API_KEY}&units=metric`;
    }
